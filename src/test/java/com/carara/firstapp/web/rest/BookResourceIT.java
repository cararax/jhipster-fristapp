...
import static com.carara.firstapp.web.rest.TestUtil.sameNumber;
...
import com.carara.firstapp.repository.BookRepository;
import com.carara.firstapp.service.criteria.BookCriteria;
import com.carara.firstapp.service.dto.BookDTO;
import com.carara.firstapp.service.mapper.BookMapper;
...
class BookResourceIT {
    ...
    @Test
    @Transactional
    void getAllBooksByAuthor_bookIsEqualToSomething() throws Exception {
        // Initialize the database
        bookRepository.saveAndFlush(book);
        Author author_book;
        if (TestUtil.findAll(em, Author.class).isEmpty()) {
            author_book = AuthorResourceIT.createEntity(em);
            em.persist(author_book);
            em.flush();
        } else {
            author_book = TestUtil.findAll(em, Author.class).get(0);
        }
        em.persist(author_book);
        em.flush();
        book.setAuthor_book(author_book);
        bookRepository.saveAndFlush(book);
        Long author_bookId = author_book.getId();

        // Get all the bookList where author_book equals to author_bookId
        defaultBookShouldBeFound("author_bookId.equals=" + author_bookId);

        // Get all the bookList where author_book equals to (author_bookId + 1)
        defaultBookShouldNotBeFound("author_bookId.equals=" + (author_bookId + 1));
    }
